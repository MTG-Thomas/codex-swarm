...
Set `CODEX_SWARM_DAEMON_URL=http://127.0.0.1:8787` or pass `cs status --daemon http://127.0.0.1:8787` to make `cs status` prefer a running daemon. `cs status` defaults to workers updated in the last 24 hours plus older non-terminal work; use `--since`, `--repo`, `--status`, and `--limit` to narrow it, `--all` for historical output, or `--json` for automation. Local and daemon-backed status report coordination coverage: active, app-server, steerable, tracker, and mock workers; active claims; message delivery state; recent file touches; and conflict messages.
